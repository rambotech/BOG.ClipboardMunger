...
namespace BOG.ClipboardMunger.Common.Helper
{
	...
	{
		public Dictionary<string, ClipboardMungerProvider> mungers = new Dictionary<string, ClipboardMungerProvider>();
		...
		public MethodRetriever()
		{
			...
			foreach (var t in mungerAssembly.GetTypes())
			{
				if (t.BaseType != null)
				{
					...
					{
						ClipboardMungerProvider o = (ClipboardMungerProvider)Activator.CreateInstance(t);
		...
		public ClipboardMungerProvider GetProviderInstance(string groupName, string methodName)
		{
			return GetProviderInstance(groupName + "::" + methodName);
		}

		public ClipboardMungerProvider GetProviderInstance(string key)
		{
